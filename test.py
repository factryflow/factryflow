...
t2 = Task(id= 2, duration= 400, resources= group2, priority=2, constraints=[r1], predecessors= [t1])
t3 = Task(id= 3, duration= 360, resources= group3, constraints=[r1], priority=4)
t4 = Task(id= 4, duration= 100, resources= group3, priority=3, constraints=[r1], predecessors=[t1, t3])
...
res_df['planned_task_start'] = res_df.apply(lambda x: int_to_datetime(
        x['task_start'], today_str), axis=1)
res_df['planned_task_end'] = res_df.apply(lambda x: int_to_datetime(
        x['task_end'], today_str), axis=1)
